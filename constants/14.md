...
$C_13$ is the smallest the smallest $n$, such the the value of the busy beaver number $BB(n)$ is undecidable in ZFC (or equivalently ZF). Explicitly, it is the smallest $n$ such that there is a Turing machine with $n$ states for which it cannot be proven in ZFC (assuming ZFC is consistent) whether it halts or not.

The existence of $C_13$ essentially follows from Gödel's second incompleteness theorem and the fact that Turing machines are strong enough to encode ZF.
...
- Scott Aaronson conjectured in [S2020] that $C_13 \leq 20$.
