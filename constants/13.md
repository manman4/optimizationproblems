...
$C_{13} = SD(\{0,1,\infty\};-1)$ is the least exponent such that one has the inequality
...
- Has many other formulations [GR2019], including an entropy formulation: $C_{13}$ is the smallest constant such that for any pair of discrete random variables $X,Y$ one has
$$ H(X,Y) \leq C_{13} \max( H(X), H(Y), H(X-Y)).$$
- In [B1999] it was observed that Kakeya sets in dimension $d$ have Minkowski and Hausdorff dimension at least $\frac{d-1}{C_{13}} + 1$. (This is no longer the best bound in any dimension.)
