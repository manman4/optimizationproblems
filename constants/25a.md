...
$C_{25}$ is the **Komlós discrepancy constant** (often denoted $K$).
...
$$
C_{25}\ :=\ \sup_{n\ge 1} K_{n}\ \in [0,\infty].
$$


The **Komlós conjecture** asserts that $C_{25}<\infty$ (i.e. $K_{n}=O(1)$ as $n\to\infty$). 

## Known upper bounds

Since it is not known whether $C_{25}$ is finite, results are typically stated as bounds on $K_{n}$.
...
| $1+\sqrt{2}$ | [Kun2023] | Best known lower bound on $C_{25}$.  |

## Additional comments and links

- **Status.** The main open problem is whether $C_{25}$ is finite (Komlós conjecture). The best known bounds currently satisfy
  $$
  1+\sqrt{2}\ \le\ C_{25}\ \le\ \infty,
  $$
...
- **[Speculation] Sharp value.** Given the lower bound $C_{25}\ge 1+\sqrt{2}$, it is natural to ask whether $C_{25}=1+\sqrt{2}$. No matching upper bound is known, and even finiteness remains open. 
